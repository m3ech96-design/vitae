...
export function ImprimiMomento({ onClose }: { onClose: () => void }) {
  ...
  // Riprende una bozza lasciata a metà (vedi lib/vitaecom-draft-context.tsx) — solo alla
  ...
  // Sincronizza continuamente la bozza — così se esci da Vitaecom con "Home" mentre stai
  ...
  const askAI = async () => {
    ...
    try {
      const res = await fetch("/api/vitaecom-caption", {
        method: "POST",
