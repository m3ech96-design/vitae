...
/**
 * Corretto secondo le istruzioni: prima ogni spesa registrata qui finiva sempre e comunque
 * addebitata dal budget del ciclo — nessun modo per segnare "questo l'ho solo annotato, non
 * deve consumare il budget" (es. un rimborso in arrivo, una spesa anticipata per qualcun
 * altro). Lo switch qui sotto lo decide al momento: "solo informativa" salva comunque
 * l'importo in cronologia (vedi lib/finance.ts) ma lo esclude dalla somma del ciclo.
 */
export function SpentPrompt({
  ...
  /** Riceve sempre la ripartizione — un solo elemento se non stai dividendo la spesa — e se
   * deve essere addebitata dal budget del ciclo o restare solo informativa. */
  onConfirm: (breakdown: SpentBreakdownItem[], chargedToBudget: boolean) => void;
  onClose: () => void;
  ...
  const confirm = () => {
    if (splitCategories) {
      if (rows.length > 0) onConfirm(rows, chargedToBudget);
    } else {
      const n = parseFloat(amount.replace(",", "."));
      if (!Number.isNaN(n) && n > 0) onConfirm([{ category: "Cibo", amount: n }], chargedToBudget);
    }
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-void-950/85 backdrop-blur-md sm:items-center">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6">
          ...
          <label className="mt-4 flex items-center justify-between gap-3 rounded-xl2 border border-white/10 bg-white/[0.03] px-3.5 py-3">
