...
export function AppointmentsSection() {
  const { appointments, addAppointment, updateAppointment, removeAppointment } = useMedical();
  ...
  const renderRow = (a: (typeof appointments)[number]) => (
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {!a.completed && (
            <>
              <button
                onClick={() => openMedicalAppointmentInCalendar(a)}
                className="focus-ring text-ink-600 hover:text-aura-cyan"
                aria-label="Aggiungi al calendario di sistema"
              >
                <CalendarPlus size={15} />
              </button>
              <button
                onClick={() => updateAppointment(a.id, { completed: true })}
                className="focus-ring text-ink-600 hover:text-aura-emerald"
  ...
  return (
    <div className="space-y-2">
      {upcoming.length === 0 && past.length === 0 && (
        <p className="py-4 text-center text-sm text-ink-800">Nessun appuntamento registrato.</p>
      )}
      ...
      {!open ? (
        ...
        <div className="space-y-2.5 rounded-xl2 border border-aura-violet/30 bg-white/[0.03] p-3">
          <TextField label="Titolo" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Es. Visita cardiologica" autoFocus />
          <div className="grid grid-cols-2 gap-2.5">
            <TextField label="Specialista (facoltativo)" value={specialist} onChange={(e) => setSpecialist(e.target.value)} />
