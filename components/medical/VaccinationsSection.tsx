    ...
    <div className="space-y-2">
          ...
          <div key={v.id} className="flex items-start justify-between rounded-xl2 border border-white/[0.06] bg-white/[0.02] px-3.5 py-2.5">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-ink-800">
                {v.date ? `Fatta il ${formatDateShort(v.date)}` : "Data non registrata"}
              </p>
              {v.nextDueDate && (
      ...
      {!open ? (
        ...
        <div className="space-y-2.5 rounded-xl2 border border-aura-violet/30 bg-white/[0.03] p-3">
          <TextField label="Nome" value={name} onChange={(e) => setName(e.target.value)} placeholder="Es. Antitetanica, influenza" autoFocus />
