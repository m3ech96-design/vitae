...
export function MedicationsSection() {
  const { medications, addMedication, removeMedication } = useMedical();
  ...
  const ongoing = medications.filter((m) => !m.endDate);
  const past = medications.filter((m) => m.endDate);

  const renderRow = (m: (typeof medications)[number]) => (
    ...
    <div className="space-y-2">
      ...
      {!open ? (
        ...
        <div className="space-y-2.5 rounded-xl2 border border-aura-violet/30 bg-white/[0.03] p-3">
          <TextField label="Nome" value={name} onChange={(e) => setName(e.target.value)} placeholder="Es. Paracetamolo" autoFocus />
          <TextField label="Dosaggio (facoltativo)" value={dosage} onChange={(e) => setDosage(e.target.value)} placeholder="Es. 500mg, 2 volte al giorno" />
          <div>
