 ...
 */
function orderTopLevelComments(comments: VitaegramComment[], authorId: string, taggedIds: Set<string>) {
  const sorted = [...comments].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  const authorFirst: VitaegramComment[] = [];
  const pinned: VitaegramComment[] = [];
  const rest: VitaegramComment[] = [];
...
function CommentRow({
  ...
  onForward: (comment: VitaegramComment) => void;
}) {
...
export function PostComments({ post, onClose }: { post: VitaegramPost; onClose: () => void }) {
  ...
  const { draft, setCommentDraft, clearDraft } = useVitaegramDraft();
  ...
  const [forwarding, setForwarding] = useState<VitaegramComment | null>(null);
