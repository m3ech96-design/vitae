...
import { useAnimalHealth, AnimalAllergy } from "@/lib/animal-health-context";
...
export function AnimalAllergiesSection({ animalId, allergies }: { animalId: string; allergies: AnimalAllergy[] }) {
  const { addAllergy, removeAllergy } = useAnimalHealth();
  ...
  const submit = () => {
    if (!name.trim()) return;
    addAllergy({ animalId, name: name.trim(), severity, reaction: reaction.trim() || undefined });
    setName("");
    ...
    <div className="space-y-2">
      ...
      {!open ? (
        ...
        <div className="space-y-2.5 rounded-xl2 border border-aura-violet/30 bg-white/[0.03] p-3">
          <TextField label="Nome" value={name} onChange={(e) => setName(e.target.value)} placeholder="Es. Pollo, acari" autoFocus />
          ...
          <TextField label="Reazione (facoltativo)" value={reaction} onChange={(e) => setReaction(e.target.value)} placeholder="Es. Prurito, vomito" />
