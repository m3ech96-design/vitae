...
export function createAuraDivIcon(
  color: string,
  opts?: { shape?: "circle" | "diamond"; size?: number; active?: boolean }
) {
  ...
  const html = `
    <div class="${ICON_WRAPPER_CLASS}" style="width:${size}px;height:${size}px;">
      ...
      <span class="relative ${shapeClass} border border-white/60" style="width:${inner}px;height:${inner}px;background:linear-gradient(135deg, ${color}, ${color}); box-shadow:0 0 14px ${color}aa, inset 0 1px 2px rgba(255,255,255,0.45);"></span>
